...
from item_system import ItemSystem, Inventory, Equipment
...
class Player:
    ...
    def add_starting_items(self, item_system):
        """Add starting items to the player"""
        # Add some basic starting items
        starting_sword = item_system.get_item('rusty_sword')
        starting_armor = item_system.get_item('leather_armor')
        health_potion = item_system.get_item('health_potion')
        
        if starting_sword:
            self.inventory.add_item(starting_sword, 1)
        if starting_armor:
            self.inventory.add_item(starting_armor, 1)
        if health_potion:
            self.inventory.add_item(health_potion, 3)
            
        Logger.info("Player: Starting items added")
    ...
    def get_effective_stats(self):
        ...
        equipment_bonuses = self.equipment.get_total_stats()
        
        return {
            'attack': self.attack + equipment_bonuses.get('attack', 0),
            'defense': self.defense + equipment_bonuses.get('defense', 0),
            'agility': self.agility + equipment_bonuses.get('agility', 0),
            'magic': self.magic + equipment_bonuses.get('magic', 0)
        }
        
    def get_effective_attack(self):
        """Get effective attack including equipment bonuses"""
        return self.attack + self.equipment.get_total_stats().get('attack', 0)
        
    def get_effective_defense(self):
        """Get effective defense including equipment bonuses"""
        return self.defense + self.equipment.get_total_stats().get('defense', 0)
    ...
    def from_dict(self, data):
        ...
        # Load inventory and equipment if available
        inventory_data = data.get('inventory')
        if inventory_data:
            self.inventory = Inventory(max_slots=inventory_data.get('max_slots', 50))
            # Load inventory items (this is simplified - in a real game you'd want more robust loading)
            
        equipment_data = data.get('equipment')
        if equipment_data:
            self.equipment = Equipment()
            # Load equipment items (this is simplified - in a real game you'd want more robust loading)
...
class GameManager:
    ...
    def start_new_game(self):
        ...
        # Add starting items
        self.player.add_starting_items(self.item_system)
    ...
    def load_game(self):
        ...
        try:
            ...
            if save_data:
                ...
                # Add starting items for new players
                self.player.add_starting_items(self.item_system)
    ...
    def complete_quest(self, quest_id):
        ...
        Returns:
            dict: Dictionary containing reward information
        """
        if not quest_id:
            return {'xp': 0, 'gold': 0, 'soul_shards': 0}
        ...
        # Mark quest as completed
        self.quest_system.complete_quest(quest_id)
        
        # Give rewards
        xp_reward = quest_data.get('xp_reward', 0)
        gold_reward = quest_data.get('gold_reward', 0)
        soul_shard_reward = quest_data.get('soul_shard_reward', 0)
        
        self.player.gain_xp(xp_reward)
        self.player.gain_gold(gold_reward)
        self.player.gain_soul_shards(soul_shard_reward)
        
        # Reset player health after quest completion
        self.player.reset_health()
        
        # Save progress
        self.save_game()
        
        Logger.info(f"GameManager: Quest {quest_id} completed. Rewards: {xp_reward} XP, {gold_reward} Gold, {soul_shard_reward} Soul Shards")
        
        return {
            'xp': xp_reward,
            'gold': gold_reward,
            'soul_shards': soul_shard_reward
        }
